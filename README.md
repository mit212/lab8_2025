...
7. Now, run `apriltag_pose.py`. Try holding up an AprilTag to the camera. You should see its 3D coordinates overlaid on the tag. If you have time, try measuring the actual distance between the AprilTag and the camera. Does it align with the distance calculated by the AprilTag detector? You may have to change the AprilTag size in line 35 of the code (the AprilTags on the white stands used in the final competition are 9.6 cm wide).
